...
@Suite struct GitIntegrationTests {
  ...
  @Test func branchOperations() async throws {
    ...
    try await repo.git.run(["push", "--quiet", "origin", "v1"])
    #expect(try await repo.git.text(["ls-remote", "--tags", "origin"]).contains("refs/tags/v1"))
    try await repo.git.deleteRemoteTag("v1", on: "origin")
    #expect(!(try await repo.git.text(["ls-remote", "--tags", "origin"]).contains("refs/tags/v1")))
    try await repo.git.deleteRemoteTag("v1", on: "origin")
